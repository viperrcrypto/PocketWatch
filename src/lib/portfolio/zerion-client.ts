...
const RETRY_MAX = 3
const RETRY_DELAYS = [2000, 4000, 8000] // escalating backoff
...
/** Fetch data for multiple wallets sequentially to avoid Zerion rate limits. */
export async function fetchMultiWalletPositions(
  ...
  // Sequential — one wallet at a time. Zerion's rate limit is very tight;
  // concurrent requests guarantee 429s for all but the first.
  for (const address of addresses) {
    try {
      ...
      failed.push(address)
      console.warn(`[zerion] Wallet ${address.slice(0, 10)}… failed: ${(err as Error).message}`)
    }
  }
