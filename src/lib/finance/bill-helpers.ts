...
export function buildCCDisplayName(cc: {
  account: { name: string; officialName: string | null; mask: string | null; institution: { institutionName: string | null } | null } | null
}): string {
  const institution = cc.account?.institution?.institutionName
  const officialName = cc.account?.officialName
  ...
  // Prefer institution + mask for clear identification
  if (institution && mask) {
    // But if we have a good official name, use it with the mask
    if (isUsefulCardName(officialName)) return `${officialName} ••••${mask}`
    return `${institution} ••••${mask}`
  }
  if (isUsefulCardName(officialName)) return officialName
