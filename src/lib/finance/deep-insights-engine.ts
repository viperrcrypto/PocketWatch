...
export async function computeDeepInsights(userId: string): Promise<DeepInsightsResult | null> {
  ...
  const txs = await db.financeTransaction.findMany({
    ...
    select: { id: true, date: true, amount: true, category: true, subcategory: true, merchantName: true, name: true, isRecurring: true, logoUrl: true },
  })

  const prevTxs = prevStart && prevEnd
    ? await db.financeTransaction.findMany({ where: { ...baseWhere, date: { gte: prevStart, lt: prevEnd } }, select: { amount: true, category: true } })
    : []
  ...
  const dailyAvg = spending / Math.max(1, dayOfMonth)
  const projectedTotal = dailyAvg * daysInMonth
  const prevSpending = prevTxs.filter((t) => t.amount > 0).reduce((s, t) => s + t.amount, 0)
  ...
  const recurringSpend = txs.filter((t) => t.amount > 0 && t.isRecurring).reduce((s, t) => s + t.amount, 0)
  ...
  // Budget health
  const budgets = await db.financeBudget.findMany({ where: { userId, isActive: true } })
  ...
  // Subscriptions
  const subs = await db.financeSubscription.findMany({ where: { userId } })
  ...
  // Query uncategorized using the same shared WHERE clause as the categorize page
  // to ensure dashboard count always matches the categorize page count
  const [uncategorizedCount, uncategorizedPreviewRaw] = await Promise.all([
