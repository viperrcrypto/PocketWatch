...
export function MerchantIcon({ logoUrl, category, size = "md" }: MerchantIconProps) {
  ...
  return (
    <div
      className={`${dims} ${roundedness} flex items-center justify-center flex-shrink-0`}
      style={{
        backgroundColor: `color-mix(in srgb, ${meta.hex} 15%, var(--background-secondary))`,
      }}
    >
      <span
        className="material-symbols-rounded"
        style={{ fontSize: iconSize, color: meta.hex }}
      >
        {meta.icon}
      </span>
    </div>
