...
import {
  ...
  useAIInsights, useGenerateAIInsights,
...
export default function InvestmentsPage() {
  ...
  const { invDelta, invPct } = useMemo(() => {
    const first = investmentChartData?.[0]
    const last = investmentChartData?.[investmentChartData.length - 1]
    const delta = first && last ? last.totalNetWorth - first.totalNetWorth : 0
    const pct = first && first.totalNetWorth !== 0 ? (delta / Math.abs(first.totalNetWorth)) * 100 : 0
    return { invDelta: delta, invPct: pct }
  }, [investmentChartData])
      ...
      <div className="space-y-6">
        ...
        <div className="bg-card border border-card-border rounded-2xl py-16 px-6 text-center">
          <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-indigo-500/20 to-purple-500/20 flex items-center justify-center mx-auto mb-6">
            <span className="material-symbols-rounded text-indigo-500" style={{ fontSize: 36 }}>trending_up</span>
          </div>
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-start gap-4">
        ...
        <div className="flex gap-3 flex-wrap">
          ...
          {/* Date Widget */}
          <div className="bg-card border border-card-border rounded-xl px-3 py-2.5 flex flex-col items-center justify-center min-w-[70px]">
            <span className="text-primary text-[10px] font-black uppercase tracking-tight">{now.toLocaleDateString("en-US", { month: "short" })}</span>
            <span className="text-2xl font-black text-foreground leading-none mt-0.5">{now.getDate()}</span>
            <span className="text-[9px] text-foreground-muted font-bold mt-0.5">{now.toLocaleDateString("en-US", { weekday: "short" }).toUpperCase()}</span>
          </div>
        </div>
      </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {investmentChartData && invDelta !== 0 && (
                    <span className={cn("text-xs font-bold font-data tabular-nums", invDelta >= 0 ? "text-success" : "text-error")}>
                      {invPct >= 0 ? "+" : ""}{invPct.toFixed(1)}% ({formatCurrency(Math.abs(invDelta))})
                    </span>
                  )}
                </div>
