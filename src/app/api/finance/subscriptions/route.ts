...
function formatAccountLabel(account: {
  ...
  const label = account.institution?.institutionName ?? account.officialName ?? account.name
  if (account.mask) {
...
export async function PATCH(req: NextRequest) {
  ...
  const { subscriptionId, status, isWanted, notes, nickname, frequency, category } = parsed.data
