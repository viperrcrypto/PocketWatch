  ...
  try {
    ...
    const accounts = accountIds.length > 0
      ? await db.financeAccount.findMany({
          ...
          select: { id: true, name: true, officialName: true, mask: true, institution: { select: { institutionName: true } } },
        })
      : []
    ...
    const result = cards.map((c) => {
      ...
      // Try officialName first, then account name, then fallback to institution + mask
      let newName = acct.officialName?.trim() ?? acct.name?.trim() ?? ""
